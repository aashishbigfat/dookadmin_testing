...
class DepartureController extends Controller
{
    ...
    public function packagesIndex(Request $request)
    {
        ...
        if ($request->status == '' && $request->keyword) {
            ...
            $departures = Departure::where('dep_type', 'package')
                //->where('tenant_id',auth()->user()->tenant_id)
                ->orderBy('id', 'DESC')
                //->distinct('departures.created_at')
                ->paginate(25);
        }
        foreach ($departures as $key => $departure_id) {
            $check_pdf = DB::table('pdf_files')
                ->where('departure_id', $departure_id->id)
                ->select('departure_id')
                ->first();
            if ($check_pdf) {
                $departure_id->pdf_status = "Exist";
            } else {
                $departure_id->pdf_status = "Not Exist";
            }

            $itinerary_day = Itinerary::where('departure_id', $departure_id->id)->count();
            $departure_id->iti_days = $itinerary_day;
        }
        $departureCount = Departure::where('dep_type', 'package')->get();
        $status = ($status == null) ? 'no' : $status;
        $total = count($departureCount);
        if ($request->ajax()) {
            return view('packages.departure_index_data', compact('departures', 'urlS3', 'keywords', 'status'));
        }
        return view('packages.departure_index', compact('departures', 'total', 'urlS3', 'keywords', 'status'));
    }
    ...
    public function packagesCreate()
    {
        ...
        return view('packages.basic_details_create', compact('IconInclusions', 'symbols', 'difficulty', 'tour_types', 'tour_classes', 'symboldollar', 'tags', 'inr'));
    }
    ...
    public function packagesStore(Request $request)
    {
        ...
        if ($unique) {
            ...
            $departure->valid_till = $request->valid_till;  // Anirudh added
    ...
    foreach ($someArray as $images) {
        if (!empty($images->Content)) {
            $base64Image = $images->Content;

            // Extract extension from base64 string header
            if (preg_match('#^data:image/(\w+);base64,#i', $base64Image, $matches)) {
                $extension = strtolower($matches[1]); // jpg, png, etc.
            } else {
                $extension = 'jpg'; // default fallback
            }

            // Strip the base64 header and decode the image
            ...
            foreach ($destArrays as $value) {
                if ($value != null) {
                    ...
                    if ($countryCheck == null) {
                        ...
                        //     $image_url = "https://s3-pullit-bucket.s3.us-west-2.amazonaws.com/country/".urlencode($value->country_image);
        ...
        $status = [
            'url' => url('/packages/inclusion', $last_id),
        ];
    ...
    public function packagesEdit(Request $request, $id)
    {
        ...
        $tour_classes = TourClass::get();
        $inr = RsDollor::value('inr');
        return view('packages.basic_details_edit', compact('departures', 'destinations', 'agent_iti_match', 'IconInclusions', 'depinclusions', 'symbols', 'urlS3', 'imagePath', 'departureimages', 'departure_difficulties', 'difficulties', 'tour_types', 'departure_tour_types', 'tour_classes', 'symboldollar', 'tags', 'departure_tags', 'inr'));
    }
    ...
    public function packagesUpdate(Request $request, $id)
    {
        ...
        $departure->price_hide_show = $request->price_hide_show;
        $departure->book_online = $request->book_online;
        $departure->price_tag = $request->price_tag;
        $departure->price_currency = $request->currency_symbol;
        $departure->price_currency_usd = $request->currency_symbol_usd;
        $departure->price = $request->price;
        $departure->valid_till = $request->valid_till;  // Anirudh added
    ...
    public function packagesDisable(Request $request, $id)
    {
        ...
        return response()->json(['success' => 'Package disabled successfully!']);
    }
     public function gccpackagesDisable(Request $request, $id)
    {
        $departures  = Departure::find($id);
        if ($departures->gcc_server == 1) {
            $departures->gcc_server = 0;
            $departures->save();
        } else {
            $departures->gcc_server = 1;
            $departures->save();
        }
    ...
    public function termConditionIndex(Request $request)
    {
        $route_ids = $request->route('id');
        $route_id = (int)$route_ids;
        $termconditionDefault = DB::table('term_and_conditions')->where('type', 'package')->first();
        // dd($termconditionDefault);
        $termconditions = Departure::select("conditions")
            ->where('id', $route_id)
            ->first();
        return view('packages.term_conditions', compact('termconditions', 'termconditionDefault'));
    }
    ...
    public function visaInformationIndex(Request $request)
    {
        ...
        // print_r($obj);
        // die();
        return view('packages.visa_information', compact('visainformation', 'countries', 'obj', 'new_data'));
    }
    ...
    public function agentItinearayGet(Request $request)
    {
        $data = [];
        if ($request->has('q')) {
            $search = $request->q;
            $data = AgentItinerary::select("id", "title")
                //->where('tenant_id', auth()->user()->tenant_id)
    ...
    public function departureOptinalActivityUpdate(Request $request)
    {
        $array_check = array();
        for ($i = 0; $i < count($request->optional); $i++) {
            if ($request->optional[$i] != '') {
                array_push($array_check, $request->optional[$i]);
            }
        }
        if ($array_check > 0) {
            DepartureOptionalActivity::where('departure_id', $request->edit_dep_id)->delete();
            foreach ($array_check as $opt_id) {
                $dep_act_update = new DepartureOptionalActivity;
                $dep_act_update->departure_id = $request->edit_dep_id;
                $dep_act_update->optional_activity_id = $opt_id;
                $dep_act_update->save();
            }
        }
    }

    public function manageBookedSeat(Request $request)
    {
        $dep_id = Departure::where('id', $request->dep_id)->value('id');
        if ($dep_id) {
            $departure_booked = Departure::find($dep_id);
            $departure_booked->booked_seat = $request->booked_seat;
            $departure_booked->save();
        }
        return response()->json(["status" => "Success!"]);
    }

    public function getOptionalAjax(Request $request)
    {
        $departure_id = $request->departure_id;
        //$departure_id = 1;
        $destinations = DB::table('departure_destinations')
            ->join('departures', 'departures.id', '=', 'departure_destinations.departure_id')
            ->join('destinations', 'destinations.id', '=', 'departure_destinations.destination_id')
            ->where('departures.id', $departure_id)
            ->select("destinations.id")
            ->get();
        foreach ($destinations as $key => $value) {
            $optional_tour[] = DB::table('optional_activities')
                ->join('destinations', 'destinations.id', '=', 'optional_activities.destination_id')
                ->where('optional_activities.destination_id', $value->id)
                ->select('optional_activities.id', 'optional_activities.title')
                ->first();
        }
    ...
    // PULL Fixed Departure Data

    public function pullIndex()
    {
        $countries = DB::table('pull_countries')->select('country_name')
            ->orderBy('country_name', 'ASC')
            ->get();
        return view('pull.pull', compact('countries'));
    }

    public function getDepartureAjax(Request $request)
    {
        $tenant = auth()->user();
        $country_name = $request->pull;
        $post = array(
        ...
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'http://52.13.174.185/departure/public/index.php/api/get_country_related_departures_details');
        curl_setopt($curl, CURLOPT_TIMEOUT, 30);
        curl_setopt($curl, CURLOPT_POST, 1);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_POSTFIELDS, $post);

        $response = curl_exec($curl); // String returns
        curl_close($curl);
        $jsons = json_decode($response); // Conver in object
        // print_r($response);
        // die;
        foreach ($jsons->data as $key => $value) {
            $agentItiCheck = AgentItinerary::where('unique_key', $value->unique_key)
                ->first();
            if ($agentItiCheck == null) {
                $agent_itinerary = new AgentItinerary;
                $agent_itinerary->title = $value->atitle;
                $agent_itinerary->reference_id = $value->aiids;
                $agent_itinerary->description = $value->aidescription;
                $agent_itinerary->pdf_file = $value->pdf_file;
                $agent_itinerary->unique_key = $value->aiukey;
                $agent_itinerary->tenant_id_reff = $value->aitenant_id;
                $agent_itinerary->tenant_id = $tenant->tenant_id;
                $agent_itinerary->user_id = $tenant->id;
                $agent_itinerary->dep_type = $value->dep_type;
                $agent_itinerary->save();
                $agent_last_id = $agent_itinerary->id;
            } else {
                $agent_ids = AgentItinerary::where('unique_key', $value->unique_key)
                    ->first();
                $departure = AgentItinerary::find($agent_ids);
                $agent_itinerary->title = $value->atitle;
                $agent_itinerary->reference_id = $value->aiids;
                $agent_itinerary->description = $value->aidescription;
                $agent_itinerary->pdf_file = $value->pdf_file;
            ...
            $departureCheck = Departure::where('unique_key', $value->unique_key)
                ->first();
            if ($departureCheck == null) {
                $departure = new Departure;
                $departure->title = $value->title;
                $departure->reference_id = $value->id;
                $departure->agent_itinerary_id = $agent_last_id;
                $departure->sub_title = $value->sub_title;
                $departure->no_of_days = $value->no_of_days;
                $departure->no_of_nights = $value->no_of_nights;
                $departure->no_of_place = $value->no_of_place;
                $departure->team_size = $value->team_size;
                $departure->transport_type = $value->transport_type;

                $departure->image = $value->image;
                $departure->flight = $value->flight;
                $departure->total_seat = $value->total_seat;
                $departure->booked_seat = $value->booked_seat;
                $departure->from = $value->from;
                $departure->date = $value->start_date;
                $departure->price = $value->price;

                $departure->meal_type = $value->meal_type;
                $departure->tour_support_team = $value->tour_support_team;
                $departure->description = $value->description;
                $departure->unique_key = $value->unique_key;
                $departure->tenant_id_reff = $value->tenant_id;
                $departure->tenant_id = $tenant->tenant_id;
                $departure->user_id = $tenant->id;
                $departure->dep_type = $value->dep_type;
                $departure->save();
                $last_id = $departure->id;
            } else {
                $dep_id = Departure::where('unique_key', $value->unique_key)
                    ->value('id');
            ...
            foreach ($value->inclusions as $key => $inclu) {
                $inclu_Check = Inclusion::where('departure_id', $last_id)
                    ->where('name', $inclu->name)
                    ->first();
                if ($inclu_Check == null) {
                ...
                //         else{
                //          $exist_id = Inclusion::where('id',$inclu_Check->id)->value('id');
                //          $inclusion  = Inclusion::find($exist_id);
                // $inclusion->name = $inclu->name;
                // $inclusion->description = $inclu->description;
                // $inclusion->departure_id = $last_id;
                // $inclusion->dep_type = $value->dep_type;
            ...
            foreach ($value->destinations as $key => $destination) {
                //Country
                $countryCheck = Country::where('country_name', $destination->country_name)
                    ->where('iso_2', $destination->iso_2)
                    ->first();
                if ($countryCheck == null) {
                    $country  = new Country;
                    $country->country_name = $destination->country_name;
                    $country->latitude = $destination->count_lat;
                    $country->longitude = $destination->count_long;
                    $country->official_name = $destination->official_name;
                    $country->capital = $destination->capital;
                    $country->largest_city = $destination->largest_city;
                    $country->continent = $destination->continent;
                    $country->description = $destination->count_des;
                    $country->sub_continent = $destination->sub_continent;
                    $country->iso_2 = $destination->iso_2;
                    $country->iso_3 = $destination->iso_3;
                    $country->isd_code = $destination->isd_code;
                    $country->internet_tld = $destination->internet_tld;
                    $country->currency = $destination->currency;
                    $country->currency_symbol = $destination->currency_symbol;
                    $country->currency_code = $destination->currency_code;
                    $country->drives_on = $destination->drives_on;
                    $country->area = $destination->area;
                    $country->area_unit = $destination->area_unit;
                    $country->population = $destination->population;
                    $country->image = $destination->count_image;
                    $country->flag = $destination->flag;
                    $country->save();
                    $country_last_id = $country->id;
                } else {
                    $count_id = Country::where('id', $countryCheck->id)->value('id');
                ...
                //destination
                $desti = Destination::where('dest_name', $destination->dest_name)
                    ->where('reference_id', $destination->reff_id)
                    ->first();
                if ($desti == null) {
                    $destinationss  = new Destination;
                    $destinationss->dest_name = $destination->dest_name;
                    $destinationss->actualname = $destination->actualname;
                    $destinationss->country_name = $destination->country_name;
                    $destinationss->country_id = $country_last_id;
                    $destinationss->reference_id = $destination->reff_id;
                    $destinationss->latitude = $destination->latitude;
                    $destinationss->longitude = $destination->longitude;
                    $destinationss->feature_class = $destination->feature_class;
                    $destinationss->feature_code = $destination->feature_code;
                    $destinationss->country_iso_2 = $destination->country_iso_2;
                    $destinationss->country_iso_3 = $destination->country_iso_3;
                    $destinationss->region_code = $destination->region_code;
                    $destinationss->region = $destination->region;
                    $destinationss->region_id = $destination->region_id;
                    $destinationss->geonameid = $destination->geonameid;
                    $destinationss->image = $destination->image;
                    $destinationss->description = $destination->description;
                    $destinationss->save();
                    $dest_last_id = $destinationss->id;
                } else {
                    $dest = Destination::where('id', $desti->id)->value('id');
                    $destinationss = Destination::find($dest);
                    $destinationss->dest_name = $destination->dest_name;
                    $destinationss->actualname = $destination->actualname;
                    $destinationss->country_name = $destination->country_name;
                    $destinationss->country_id = $country_last_id;
                ...
                $destdep = DepartureDestination::where('destination_id', $dest_last_id)
                    ->where('departure_id', $last_id)
                    ->first();
                if ($destdep == null) {
                    $depdestination  = new DepartureDestination;
                    $depdestination->departure_id = $last_id;
                ...
                // Destination-Departure POIS
                foreach ($destination->pois as $key => $poi) {
                    $poi_check = DepartureDestinationPointOfInterest::where(['departure_id' => $last_id, 'destination_id' => $dest_last_id, 'reference_id' => $poi->reference_id])
                        ->first();
                    if ($poi_check == null) {
                        $point_of_interest = new DepartureDestinationPointOfInterest;
                        $point_of_interest->poi_name = $poi->poi_name;
                        $point_of_interest->poi_type = $poi->poi_type;
                        $point_of_interest->rating = $poi->rating;
                        $point_of_interest->reference_id = $poi->reference_id;
                        $point_of_interest->latitude = $poi->latitude;
                        $point_of_interest->longitude = $poi->longitude;
                        $point_of_interest->image = $poi->image;
                        $point_of_interest->address = $poi->address;
                        $point_of_interest->destination_id = $dest_last_id;
                        $point_of_interest->departure_id = $last_id;
                        $point_of_interest->dep_type = $value->dep_type;
                        $point_of_interest->tenant_id_reff = $value->tenant_id;
                        $point_of_interest->tenant_id = $tenant->tenant_id;
                        $point_of_interest->user_id = $tenant->id;
                        $point_of_interest->save();
                    } else {
                        $poi_id = DepartureDestinationPointOfInterest::where(['departure_id' => $last_id, 'destination_id' => $dest_last_id, 'reference_id' => $poi->reference_id])->value('id');

                        $point_of_interest = DepartureDestinationPointOfInterest::find($poi_id);
                        $point_of_interest->poi_name = $poi->poi_name;
                        $point_of_interest->poi_type = $poi->poi_type;
                        $point_of_interest->rating = $poi->rating;
                        $point_of_interest->reference_id = $poi->reference_id;
                ...
                foreach ($destination->experiences as $key => $exp) {
                    $exp_check = Experience::where('experience_name', $exp->experience_name)
                        ->first();
                    if ($exp_check == null) {
                        $experience = new Experience;
                        $experience->experience_name = $exp->experience_name;
                        $experience->image = $exp->image;
                        $experience->save();
                        $exp_last_id = $experience->id;
                    } else {
                        $exp_id = Experience::where('experience_name', $exp->experience_name)
                            ->value('id');
                        $experience = Experience::find($exp_id);
                        $experience->experience_name = $exp->experience_name;
                        $experience->image = $poi->image;
                        $experience->save();
                        $exp_last_id = $experience->id;
                    }

                    $destexp = DestinationExperience::where('destination_id', $dest_last_id)
                        ->where('experience_id', $exp_last_id)
                        ->first();
                    if ($destexp == null) {
                        $expdestination  = new DestinationExperience;
                        $expdestination->experience_id = $exp_last_id;
                        $expdestination->destination_id = $dest_last_id;
                        $expdestination->save();
                    }
                }
            }
            // Itineraries
            foreach ($value->itineraries as $key => $itinary) {
                $inclu_Check = Itinerary::where('departure_id', $last_id)
                    ->where('unique_key', $itinary->unique_key)
                    ->first();
                if ($inclu_Check == null) {
                    $itinerary  = new Itinerary;
                    $itinerary->day_number = $itinary->day_number;
                    $itinerary->day_heading = $itinary->day_heading;
                    $itinerary->included = $itinary->included;
                    $itinerary->excluded = $itinary->excluded;
                    $itinerary->description = $itinary->description;
                    $itinerary->unique_key = $itinary->unique_key;
                    $itinerary->departure_id = $last_id;
                    $itinerary->dep_type = $value->dep_type;
                    $itinerary->tenant_id_reff = $value->tenant_id;
                    $itinerary->tenant_id = $tenant->tenant_id;
                    $itinerary->user_id = $tenant->id;
                    $itinerary->save();
                    $itin_last_id = $itinerary->id;
                } else {
                    $iti_id = Itinerary::where('departure_id', $last_id)
                        ->where('unique_key', $itinary->unique_key)
                        ->value('id');
                ...
                foreach ($itinary->destinationpoi as $key => $itidestpoi) {

                    $get_destination = Destination::where('reference_id', $itidestpoi->dest_reff_id)
                        ->where('dest_name', $itidestpoi->dest_name)
                        ->select('id')
                        ->first();

                    $DdiP_Check = DB::table('destination_itinerary_point_of_interests')
                        ->where(['itinerary_id' => $itin_last_id, 'point_of_interest_id' => $itidestpoi->poi_reff_id, 'departure_id' => $last_id, 'destination_id' => $get_destination->id])
                        ->first();
                    if ($DdiP_Check == null) {
                        $dest_itinerary_poi = new DestinationItineraryPointOfInterest;
                        $dest_itinerary_poi->departure_id = $last_id;
                        $dest_itinerary_poi->itinerary_id = $itin_last_id;
                        $dest_itinerary_poi->destination_id = $get_destination->id;
                        $dest_itinerary_poi->point_of_interest_id = $itidestpoi->poi_reff_id;
                        $dest_itinerary_poi->save();
                    }
                }
            }
        }
    ...
    public function popularPackagesIndex(Request $request)
    {
        ...
        foreach ($departures as $key => $departure) {
            $deprture_row = DB::table('departure_optional_activities')
                ->where('departure_id', $departure->id)
                ->pluck('optional_activity_id')
                ->toArray();
            $departure['optional_activity_id'] = $deprture_row;

            $optional_activity_name = array();
            foreach ($deprture_row as $key => $value) {
                $opt_activity_name = DB::table('optional_activities')
                    ->where('id', $value)
                    ->first();
                array_push($optional_activity_name, $opt_activity_name->title);
            }
            $departure['optional_activity_title'] = $optional_activity_name;
        }
        $departureCount = Departure::where('dep_type', 'package')->get();
        $total = count($departureCount);
        if ($request->ajax()) {
            return view('packages.departure_index_data', compact('departures'));
        }
        return view('packages.departure_index', compact('departures', 'total'));
    }

    // Package Id Unique Check Live for all group, Packages and departures
    public function liveValidationUniqueIdCheck(Request $request)
    {
        //print_r($request->get('pkg_id'));die;
        if ($request->get('pkg_id')) {
            $unique_id = $request->get('pkg_id');
            $id_count = Departure::where('dep_dook_ref_id', $unique_id)
                ->count();
            if ($id_count > 0) {
                echo 'not-unique';
            } else {
    ...
    public function packageShowAtHome(Request $request, $id)
    {
        $popular  = Departure::find($id);

        if ($popular->popular_at_home == 1) {
            $popular->popular_at_home = 0;
            $popular->save();
        } else {
            $popular->popular_at_home = 1;
            $popular->save();
        }
